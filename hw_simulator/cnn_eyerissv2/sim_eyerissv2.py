...
def run_sim(args):
  ...
  with torch.no_grad():
    ...
    for dataset_path in dataset_paths:
      ...
      for step, (x, y) in tqdm(enumerate(test_loader), total=args.num_samples):
        x.cpu()
        output = model(x)
        ...
        cur_batch_num += 1
        if (cur_batch_num % args.num_samples == 0): break # Only run num_samples for each dataset.
...
if __name__ == '__main__':
  ...
  parser.add_argument("--num_samples", default=100, type=int, help="The number of samples on each dataset")

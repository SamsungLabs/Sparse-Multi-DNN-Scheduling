from utils_common import Task, take
import logging
import numpy as np
...
class Scheduler:
  ...
  def calc_violation_rate(self):
    ...
    # Sort the violated task dict according to request time
  ...
  def calc_system_thrpt(self):
    ...
    first_reqst_time, _, _, _, _, _, _, _ = self.reqst_table[0]
  ...
  def calc_ANTT(self):
    """
    Returns the average normalised turnaround time (ANTT) 
    across all inference tasks. This metric captures the average slowdown
    with respect to the isolated execution of each task on the same hardware.
    """
    assert self.is_finished()
    task_ntts = []
    for task_id,task_info in self.finished_reqst.items():
  ...
  def __push_reqst(self, reqst_indx, is_pattern_aware=False):
    # Read the new input request
    reqst_time, target_lat, reqst_model, priority, avg_lat, sample_id, avg_sparsity, avg_lat_per_pattern = self.reqst_table[reqst_indx]
    
    # Construct a new task
    new_task = Task(reqst_time, target_lat, reqst_model, priority, avg_lat, avg_sparsity, avg_lat_per_pattern, is_pattern_aware)
  ...
  def update_reqst(self, is_pattern_aware=False):
    if (self.cur_reqst_indx < self.num_reqst): 
      # request info: req_time, target_lat, model, priority
      reqst_time, target_lat, reqst_model, priority, avg_lat, sample_id, avg_sparsity, avg_lat_per_pattern = self.reqst_table[self.cur_reqst_indx]
      
      # No running task, push the nearest request
      if (len(self.running_task) == 0): 
        self.__push_reqst(self.cur_reqst_indx, is_pattern_aware)
...
class PREMA_Scheduler(Scheduler):
  ...
  def __init__(self,reqst_table, is_sparse=False):
    ...
    print ("Constructing PREMA Scheduler with sparse estimator %s."%(self.is_sparse))
  ...
  def update_schedule(self):
    ...
    for task_id,task in self.running_task.items():
      ...
      # Initialize tokens with priorities
      if (task.prema_token < 0): 
        ...
        if self.is_sparse:
          ...
          slowdown_rate = idle_time / (task.dysta_gamma * task.est_isolated_time)
        else:
          slowdown_rate = idle_time / task.est_isolated_time
    ...
    for task_id in candidate_tasks:
      if self.is_sparse:
        ...
        estimated_time = task.dysta_gamma * sum(self.running_task[task_id].est_lat_queue)
      else:
...
class Dysta_Scheduler(Scheduler):
  ...
  """
  def __init__(self,reqst_table, penalty_eff=1.0, num_candidate=5, beta=0.01, is_sparse=True):
    super().__init__(reqst_table)
    print ("Constructing Dysta HW/SW Scheduler with sparse estimator %s."%(is_sparse))
  ...
  def update_schedule(self):
    ...
    for task_id,task in self.running_task.items():
      ...
      #     1. Avoid task switch as it causes extra resource consumption
      ...
      # penalty_preemption = idle_time / ( task.dysta_gamma * task.est_isolated_time )
      penalty_preemption = idle_time / task.est_isolated_time
    ...
    # Get next token according to shortest estimated time
    shortest_time = -1
    for task_id, task in self.running_task.items():
      if self.is_sparse:
        estimated_time = task.dysta_gamma * sum(task.est_lat_queue) # Use latency estimator
      else:
        estimated_time = sum(task.est_lat_queue)
...
class Dysta_Oracle_Scheduler(Scheduler):
  ...
  def update_schedule(self):
    ...
    for task_id,task in self.running_task.items():
      ...
      # Comment out because these metrics do not help. But keep this, in case we need it in the future.
      '''
...
class SDRM3_Scheduler(Scheduler):
  ...
  """
  def __init__(self,reqst_table, alpha=1.0, is_sparse=False):
    super().__init__(reqst_table)
    self.alpha = alpha
    self.is_sparse = is_sparse
    print ("Constructing SDRM3 Scheduler with %s."%(self.is_sparse))
  ...
  def update_schedule(self):
    ...
    for task_id,task in self.running_task.items():
      ...
      idle_time = self.sys_time - task.last_exe_time
      if self.is_sparse:
        task.fairness = idle_time / (task.dysta_gamma * task.est_lat_queue[0])
        # task.fairness = idle_time / task.real_lat_queue[0]
      else:
  ...
  def update_schedule(self):
    ...
    for task_id, task in self.running_task.items():
      if self.is_sparse:
        # estimated_time = sum(self.running_task[task_id].real_lat_queue) # Use real sparsity to estimate lat
        estimated_time = task.dysta_gamma * sum(self.running_task[task_id].est_lat_queue)
        # real_time = sum(self.running_task[task_id].real_lat_queue)
        # rel_error = abs(estimated_time - real_time)/real_time
        # if ( rel_error > 0.1): print ("Not accurate prediction with error %f on task %s"%(rel_error, task_id))
      else:
        estimated_time = sum(self.running_task[task_id].est_lat_queue) # Use estimate avg (PREMA) lat  
      slack_time =  task.target_time - self.sys_time
      torun_lat = sum(task.real_lat_queue)
